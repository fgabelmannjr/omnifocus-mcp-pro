 ...
 * This clears both `reviewInterval` and `nextReviewDate`.
 *
 * ## Value Object Semantics (Critical)
 *
 * ReviewInterval is a value object - must reassign entirely:
 *
 * ```javascript
 * // WRONG - modifies local copy only
 * project.reviewInterval.steps = 14;
 *
 * // CORRECT - full reassignment triggers update
 ...
 * - If `nextReviewDate` is null, it's calculated as `today + interval`
 * - If `nextReviewDate` already exists, it's preserved (not recalculated)
 *
 * When interval is set to null:
 * - `reviewInterval` is cleared
 * - `nextReviewDate` is cleared
 * - Project no longer appears in review queries
 */
